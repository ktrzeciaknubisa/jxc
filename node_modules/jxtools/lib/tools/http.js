...
process.on('exit', function() {
  ...
  //consoleEx.debugInfo('process exit http', cleanupFiles);

  for(var o in cleanupFiles) {
    if (!cleanupFiles.hasOwnProperty(o))
      continue;

    try {
      consoleEx.debugInfo('process exit cleanupFiles', o);
      if (fs.existsSync(o)) {
        fs.unlinkSync(o);
        consoleEx.debugInfo('Deleted', o);
      }
...
var timeouter = function(ms) {
  ...
  this.start = function() {
    consoleEx.debugInfo('timeouter', _this.id, 'started');
    _this.reset();
  ...
  this.stop = function() {
    consoleEx.debugInfo('timeouter', _this.id, 'stopped');
    stopped = true;
  };
...
var downloader = function (uri, localFile, options) {
  ...
  var _exit = function (err, tryNext, result) {
    ...
    consoleEx.debugInfo('downloader _exit called', err ? 'with err ' + err : 'without error');
    ...
    process.nextTick(function() {
      ...
      if (!err) {
        ...
        //  consoleEx.debugInfo('downloader renaming', tmpFile, 'to', localFile);
        //  consoleEx.debugInfo(fs.existsSync(localFile) && !fs.existsSync(tmpFile) ? 'OK' : 'did not rename?');
        //}

        delete cleanupFiles[localFile];
      }
    ...
    if (options.timeout) {
      ...
      tm.on('timeout', function() {
        if (!__error) {
          ...
          //try {
          //  res.unpipe(file, function(xxx) {
          //    consoleEx.debugInfo('downloader unpiped file', xxx);
          //  });
          //} catch (ex) {
          //  consoleEx.debugInfo('downloader cannot unpipe', ex);
          //}

          if (req) {
            try {
              req.abort();
              consoleEx.debugInfo('downloader aborted request');
            } catch (ex) {}
            //try {
            //  req.close();
            //  consoleEx.debugInfo('downloader closing request');
            //} catch (ex) {}
            //try {
            //  res.abort();
            //  consoleEx.debugInfo('downloader aborted response');
            //} catch (ex) {}
