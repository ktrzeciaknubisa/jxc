  ...
  for(var o in files) {
    ...
    if (files.hasOwnProperty(o) && f.slice(0,8) === tpmFilePrefix) {
      try {
        if (fs.existsSync(f)) {
          fs.unlinkSync(f);
          consoleEx.debugInfo('process exit cleanupFiles', f);
        }
      } catch (ex) {
      }
    }
...
var downloader = function (uri, localFile, options) {
  ...
  var _exit = function (err, tryNext, result) {
    ...
    //consoleEx.debugInfo("Attempt", attemptId, "/", attempts);
  ...
  var _download = function () {
    ...
    if (localFile) {
      ...
      file.on('finish', function () {
        if (__error) return;
        consoleEx.debugInfo('downloader response end: closing file', tmpFile);
        file.close();

        var stat = fs.statSync(tmpFile);
        consoleEx.debugInfo('downloader fetched file size of', tmpFile, '=', stat.size);
